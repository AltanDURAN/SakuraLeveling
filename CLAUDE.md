...
- **Bannière `/profile`** ([`profile_banner.py`](app/bot/rendering/profile_banner.py)) : Pillow génère un PNG 1024×800 contenant TOUT le profil (header avec avatar/title/level/XP-bar/class + badge de rang coloré + power score + 8 stats de combat + 8 stats de carrière incl. dodges_total + V/D/duel_rank). L'embed Discord ne contient que `set_image(attachment://...)` — pas de fields, pas de duplication. La barre d'XP utilise `ProgressionService.xp_required_for_next_level(level)` pour afficher `XP : x / 100×level (pct%)`. Pas d'emojis dans le PNG (DejaVuSans n'a pas les glyphes color emoji) : on utilise des symboles Unicode supportés (★ ◆) ou du texte. Si la génération échoue, fallback silencieux sur `build_player_profile_embed` (ancien embed avec fields). Les fichiers sont stockés dans `assets/generated_profiles/` (gitignoré).
